        ...
        if request.method == 'POST':
            ...
            if (first != '') or (last != '') or (phone != ''):
                ...
                if (first != '') and (last == '') and (phone == ''):
                    search_list = Profile.objects.filter(FK_User__in = User.objects.filter(Q(first_name__icontains = first)))
                elif (last != '') and (first == '') and (phone == ''):
                    search_list = Profile.objects.filter(FK_User__in = User.objects.filter(Q(last_name__icontains = last)))
                elif (phone != '') and (first == '') and (last == ''):
                    search_list = Profile.objects.filter(Q(MobileNumber__icontains = phone))
                elif (first != '') and (last != '') and (phone == ''):
                    search_list = Profile.objects.filter(FK_User__in = User.objects.filter(Q(first_name__icontains = first, last_name__icontains = last)))
                elif (first != '') and (phone != '') and (last == ''):
                    search_list = Profile.objects.filter(FK_User__in = User.objects.filter(Q(first_name__icontains = first)))
                    search_list = search_list.filter(Q(MobileNumber__icontains = phone))
                ...
                if search_list != None:
                    for item in search_list:
                        AllProfile.append(item)

                AllProfile = list(dict.fromkeys(AllProfile))
                # ----------------------------------------------------------------------
                # Get User Info
                ...
                # Build User Class
                class UserClass:
                    def __init__(self,  id, username, firstname, lastname, mobilenumber, email, status):
                        self.id = id
                ...
                # Get All User
                for user_profile in AllProfile:
                    New_Item = UserClass(user_profile.FK_User.id, user_profile.FK_User.username, user_profile.FK_User.first_name, user_profile.FK_User.last_name, user_profile.MobileNumber, user_profile.FK_User.email, user_profile.FK_User.is_active)
                    AllUserList.append(New_Item)
            ...
            AllUserListPaginator = Paginator (AllUserList, 10)
            page = request.GET.get('page')

            AllUserList = AllUserListPaginator.get_page(page)
            ...
            print('all users: {}'.format(time.time()-t0))
            return render(request, 'nakhll_market/management/content/show_all_user_info.html', context)
...
def Show_All_Shoper_User_Info(request):
    ...
    if request.user.is_authenticated :
        ...
        # Get Profile
        for user_profile in Shop_Profile:
            New_Item = UserClass(user_profile.FK_User.id, user_profile.FK_User.username, user_profile.FK_User.first_name, user_profile.FK_User.last_name, user_profile.MobileNumber, user_profile.FK_User.email, user_profile.FK_User.is_active)
            AllUserList.append(New_Item)
