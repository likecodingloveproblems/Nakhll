...
from accounting_new.serializers import InvoiceRetrieveSerializer, InvoiceItemSerializer, OnlyInvoiceSerializer
...
def get_transactions(request):
    try:
        transaction = Transaction.objects.filter(
            FK_User=request.user).order_by('-Date')
    ...
    try:
        if Shop.objects.filter(FK_ShopManager=request.user).exists():
            ...
            if user_factor_item:
                ...
                item_serializer = FactorPostSerializer(
                    user_factor_item, many=True)
...
def get_shop_detail(request):
    ...
    if Shop.objects.filter(ID=request.POST.get('shop_id'), FK_ShopManager=request.user).exists():
        ...
        shop = Shop.objects.filter(ID=request.POST.get(
            'shop_id'), FK_ShopManager=request.user)[0]
...
def create_new_shop(request):
    if BankAccount.objects.filter(FK_Profile=Profile.objects.get(FK_User=request.user)).exists():
        ...
        try:
            ...
            Shop_Avatar = ''
        try:
            shop = Shop.objects.create(FK_ShopManager=request.user, Title=Shop_Title,
                                       Slug=Shop_Slug, State=Shop_State, BigCity=Shop_BigCity, City=Shop_City)
            # Set Shop Bio
            if Shop_Bio != '':
                shop.Bio = Shop_Bio
...
def add_shop_banner(request):
    if Shop.objects.filter(ID=request.POST.get('shop_id'), FK_ShopManager=request.user).exists():
        ...
        if (Banner_Title != False) and (Banner_Image != False):
            thisbanner = ShopBanner(FK_Shop=Shop.objects.get(
                ID=Shop_id), Title=Banner_Title, Image=Banner_Image)
            thisbanner.save()
...
def create_new_product(request):
    try:
        if Shop.objects.filter(ID=request.POST.get('product_shop_id'), FK_ShopManager=request.user).exists():
            ...
            try:
                Exception_Post_Range = request.POST.get(
                    "product_exception_post_range")
            except:
            ...
            # Create New Product
            this_product = Product.objects.create(Title=Title, Slug=Slug, FK_SubMarket=SubMarket.objects.get(
                ID=This_SubMarket), Image=Image, FK_Shop=Shop.objects.get(ID=This_Shop), Price=Price, Status=Status)
            ...
            for item in CategoryList.split('~'):
                try:
                    if Category.objects.filter(id=item).exists():
                        this_product.FK_Category.add(
                            Category.objects.get(id=item))
                except:
                    continue
            ...
            for item in Post_Range_List:
                try:
                    if PostRange.objects.filter(id=item).exists():
                        this_product.FK_PostRange.add(
                            PostRange.objects.get(id=item))
                except:
                    continue
            ...
            for item in Exception_Post_Range_List:
                try:
                    if PostRange.objects.filter(id=item).exists():
                        this_product.FK_ExceptionPostRange.add(
                            PostRange.objects.get(id=item))
                except:
                    continue
            ...
            this_product.save()
            # Set Alert
            Alert.objects.create(FK_User=request.user,
                                 Part='6', Slug=this_product.ID)
...
def edit_user_shop(request):
    if Shop.objects.filter(FK_ShopManager=request.user, Slug=request.POST.get('shop_slug')).exists():
        ...
        try:
            # Get This Shop
            this_shop = get_object_or_404(
                Shop, Slug=request.POST.get('shop_slug'))
            alert = None
            # Set Alert
            if Alert.objects.filter(Part='3', Slug=this_shop.ID, Seen=False).exists():
                alert = get_object_or_404(
                    Alert, Part='3', Slug=this_shop.ID, Seen=False)
                alert.FK_Field.all().delete()
                alert.save()
            else:
                alert = Alert.objects.create(
                    FK_User=request.user, Part='3', Slug=this_shop.ID)
            # Set This_Submarket
            if Shop_SubMarkets != '':
                SubMarketField = Field(
                    Title='SubMarket', Value=Shop_SubMarkets)
                SubMarketField.save()
            ...
            if (Banner_Title != ''):
                ...
                if (Banner_Title != thisbanner.Title) or (Banner_Image != '') or (Banner_URL != thisbanner.URL):
                    ...
                    if Alert.objects.filter(Part='5', Slug=thisbanner.id, Seen=False).count() == 0:
                        alert = Alert(FK_User=request.user,
                                      Part='5', Slug=thisbanner.id)
                        alert.save()
                    else:
                        alert = Alert.objects.get(
                            Part='5', Slug=thisbanner.id, Seen=False)
                        alert.FK_Field.all().delete()
                        alert.save()
    ...
    try:
        if Shop.objects.filter(FK_ShopManager=request.user).exists():
            ...
            Alert.objects.create(
                Part='22', FK_User=request.user, Slug=banner.id)
    ...
    if Product.objects.filter(ID=request.POST["product_id"]).exists():
        ...
        if Shop.objects.filter(ID=this_product.FK_Shop.ID, FK_ShopManager=request.user).exists():
            ...
            if Alert.objects.filter(Part='7', Slug=this_product.ID, Seen=False).exists():
                alert = get_object_or_404(
                    Alert, Part='7', Slug=this_product.ID, Seen=False)
                alert.FK_Field.all().delete()
            else:
                alert = Alert.objects.create(
                    FK_User=request.user, Part='7', Slug=this_product.ID)
            ...
            if (This_SubMarket != '') and (SubMarket.objects.filter(ID=This_SubMarket).exists()):
                if get_object_or_404(SubMarket, ID=This_SubMarket) != this_product.FK_SubMarket:
                    SubMarketField = Field(
                        Title='SubMarket', Value=This_SubMarket)
                    SubMarketField.save()
                    alert.FK_Field.add(SubMarketField)
            ...
            if Status == '1':
                if (Store_Inventory != '') and (int(Store_Inventory) != this_product.Inventory):
                    ProdInStoreField = Field(
                        Title='ProdInStore', Value=Store_Inventory)
            ...
            if len(Exception_Post_Range_List) != 0:
                ...
                ExePostRangeField = Field(
                    Title='ExePostRange', Value=Exe_Post_Range_txt)
            ...
            if Post_Range_Type != this_product.PostRangeType:
                ProdRangeField = Field(
                    Title='ProdRange', Value=Post_Range_Type)
                ProdRangeField.save()
                alert.FK_Field.add(ProdRangeField)
            # Product Weight Info
            if (Net_Weight != '') and (Net_Weight != this_product.Net_Weight):
                ProdNet_WeightField = Field(
                    Title='ProdNetWeight', Value=Net_Weight)
            ...
            if (Packing_Weight != '') and (Packing_Weight != this_product.Weight_With_Packing):
                ProdPacking_Weight = Field(
                    Title='ProdPackingWeight', Value=Packing_Weight)
            ...
            if (Length_With_Packaging != '') and (Length_With_Packaging != this_product.Length_With_Packaging):
                ProdLength_With_Packaging = Field(
                    Title='ProdLengthWithPackaging', Value=Length_With_Packaging)
            ...
            if (Width_With_Packaging != '') and (Width_With_Packaging != this_product.Width_With_Packaging):
                ProdWidth_With_Packaging = Field(
                    Title='ProdWidthWithPackaging', Value=Width_With_Packaging)
            ...
            if (Height_With_Packaging != '') and (Height_With_Packaging != this_product.Height_With_Packaging):
                ProdHeight_With_Packaging = Field(
                    Title='ProdHeightWithPackaging', Value=Height_With_Packaging)
        ...
        if Shop.objects.filter(ID=this_product.FK_Shop.ID, FK_ShopManager=request.user).exists():
            ...
            try:
                if (Attribute.objects.filter(id=Attribute_id).exists()) and (Value != ''):
                    ...
                    attrproduct = AttrProduct.objects.create(
                        FK_Product=this_product, FK_Attribute=this_attribute, Value=Value)
                    # Set Alert
                    Alert.objects.create(
                        FK_User=request.user, Part='11', Slug=attrproduct.id)
    ...
    if Shop.objects.filter(FK_ShopManager=request.user).exists():
        ...
        try:
            if (Attribute_Title != '') and (Attribute_Value != '') and not (Attribute.objects.filter(Title=Attribute_Title, Unit=Attribute_Value).exists()):
                this_attribute = Attribute.objects.create(
                    Title=Attribute_Title, Unit=Attribute_Value)
                Alert.objects.create(FK_User=request.user,
                                     Part='10', Slug=this_attribute.id)
...
def delete_product_attribute(request):
    ...
    if AttrProduct.objects.filter(id=request.POST["attribute_id"]).exists():
        try:
            ...
            this_attrProduct = get_object_or_404(
                AttrProduct, id=request.POST["attribute_id"])
            if Shop.objects.filter(FK_ShopManager=this_attrProduct.FK_Product.FK_Shop.FK_ShopManager.id).exists():
                ...
                # Set Alert
                Alert.objects.create(
                    Part='24', FK_User=request.user, Slug=this_attrProduct.id)
...
def get_product_attribute(request):
    if Product.objects.filter(ID=request.POST["product_id"]).exists():
        ...
        if Shop.objects.filter(ID=this_product.FK_Shop.ID, FK_ShopManager=request.user).exists():
            try:
                product_attribute_list = AttrProduct.objects.filter(
                    FK_Product=this_product, Available=True)
                serializer = AttrProductSerializer(
                    product_attribute_list, many=True)
                return JsonResponse(serializer.data, status=HTTP_200_OK, safe=False)
            except Exception as e:
                return JsonResponse({'res': str(e)}, status=HTTP_400_BAD_REQUEST)
        else:
...
def get_product_price_attribute(request):
    if Product.objects.filter(ID=request.POST["product_id"]).exists():
        ...
        if Shop.objects.filter(ID=this_product.FK_Shop.ID, FK_ShopManager=request.user).exists():
            try:
                product_price_attribute_list = AttrPrice.objects.filter(
                    FK_Product=this_product, Publish=True)
                serializer = AttrPriceSerializer(
                    product_price_attribute_list, many=True)
                return JsonResponse(serializer.data, status=HTTP_200_OK, safe=False)
            except Exception as e:
                return JsonResponse({'res': str(e)}, status=HTTP_400_BAD_REQUEST)
        else:
...
def change_product_price_attribute_status(request):
    if AttrPrice.objects.filter(id=request.POST["price_attribute_id"]).exists():
        ...
        this_price_attribute = AttrPrice.objects.get(
            id=request.POST["price_attribute_id"])
        # Get This Product
        ...
        if Shop.objects.filter(ID=this_product.FK_Shop.ID, FK_ShopManager=request.user).exists():
            ...
            try:
                if (AttrPrice_Value != '') and (AttrPrice_Exp != '') and (AttrPrice_Unit != '') and not (AttrPrice.objects.filter(FK_Product=this_product, Value=AttrPrice_Value, ExtraPrice=AttrPrice_Exp, Unit=AttrPrice_Unit).exists()):
                    if AttrPrice_Des == '':
                        this_price_attribute = AttrPrice.objects.create(
                            FK_Product=this_product, Value=AttrPrice_Value, ExtraPrice=AttrPrice_Exp, Unit=AttrPrice_Unit, Publish=False)
                    else:
                        this_price_attribute = AttrPrice.objects.create(
                            FK_Product=this_product, Value=AttrPrice_Value, ExtraPrice=AttrPrice_Exp, Unit=AttrPrice_Unit, Description=AttrPrice_Des, Publish=False)
                    Alert.objects.create(
                        Part='17', FK_User=request.user, Slug=this_price_attribute.id)
                    return JsonResponse({'res': 'True'}, status=HTTP_201_CREATED)
                else:
    ...
    if AttrPrice.objects.filter(id=request.POST["price_attribute_id"]).exists():
        ...
        if Shop.objects.filter(ID=this_product.FK_Shop.ID, FK_ShopManager=request.user).exists():
            try:
                ...
                # Set Alert
                Alert.objects.create(
                    Part='25', FK_User=request.user, Slug=this_price_attribute.id)
...
def get_product_all_banner(request):
    if Product.objects.filter(ID=request.POST["product_id"]).exists():
        ...
        if Shop.objects.filter(ID=this_product.FK_Shop.ID, FK_ShopManager=request.user).exists():
            try:
                this_product_banner_list = ProductBanner.objects.filter(
                    FK_Product=this_product, Publish=True)
                serializer = ProductBannerSerializer(
                    this_product_banner_list, many=True)
                return JsonResponse(serializer.data, status=HTTP_200_OK, safe=False)
            except Exception as e:
                return JsonResponse({'res': str(e)}, status=HTTP_400_BAD_REQUEST)
        else:
...
def add_product_banner(request):
    if Product.objects.filter(ID=request.POST["product_id"]).exists():
        ...
        if Shop.objects.filter(ID=this_product.FK_Shop.ID, FK_ShopManager=request.user).exists():
            ...
            try:
                if Banner_Image != '':
                    this_banner = ProductBanner.objects.create(
                        FK_Product=this_product, Image=Banner_Image)
                    # Set Alert
                    Alert.objects.create(
                        FK_User=request.user, Part='8', Slug=this_banner.id)
...
def barcodepost_for_factor(request):
    ...
    pubproduc = Product.objects.filter(
        FK_Shop__in=Shop.objects.filter(FK_ShopManager=request.user))
    ...
    if len(factors_post_status_three) != 0:
        ...
        if (Barcode_Send != '') and (len(Barcode_Send) == 24) and (Price_Send != '') and (User_Send != '') and (Factor_id != ''):
            if (PostBarCode.objects.filter(FK_Factor=Factor.objects.get(ID=Factor_id), BarCode=Barcode_Send, User_Sender=User_Send).count() == 0):
                # Add Barcode
                barcode = PostBarCode.objects.create(FK_Factor=Factor.objects.get(
                    ID=Factor_id), User_Sender=User_Send, PostPrice=Price_Send, BarCode=Barcode_Send)
                # Set Alert
                Alert.objects.create(
                    Part='21', FK_User=request.user, Slug=barcode.id)
...
def accept_factor_product(request):
    ...
    try:
        ...
        if Alert.objects.filter(Part='20', FK_User=request.user, Slug=Factor_id).exists():
            ...
            Alert.objects.create(
                Part='20', FK_User=request.user, Slug=Factor_id)
...
def cancel_factor_product(request):
    ...
    try:
        ...
        if Alert.objects.filter(Part='13', FK_User=request.user, Slug=factor.ID).exists():
            ...
            Alert.objects.create(
                Part='13', FK_User=request.user, Slug=factor.ID)
...
def change_message_status(request):
    ...
    for item in Message.objects.filter(Type=True).order_by('-Date'):
        try:
            if item.FK_Users.filter(FK_User=request.user, SeenStatus=False).exists():
                this = item.FK_Users.get(
                    FK_User=request.user, SeenStatus=False)
...
def get_factor_products(request):
    try:
        user_factor = Factor.objects.filter(
            FK_User=request.user, PaymentStatus=False)[0]
...
def add_to_basket(request):
    ...
    if request.user.is_authenticated:
        ...
        if item.Status != '4':
            Factor_qs = Factor.objects.filter(
                FK_User=request.user, PaymentStatus=False)
            if Factor_qs.exists():
                order = Factor_qs[0]
                try:
                    ...
                    Factor_item = FactorPost(
                        FK_Product=item, FK_User=request.user)
                    Factor_item.save()
...
def add_check_out(request):
    try:
        ...
        factors_post_status_three = Factor.objects.filter(
            PaymentStatus=True, Publish=True, FK_FactorPost__in=factor_items_post_status_three)
        factors_post_status_three = list(
            dict.fromkeys(factors_post_status_three))
        ...
        if len(factors_post_status_three) != 0:
            ...
            Alert.objects.create(
                Part='31', FK_User=request.user, Slug=Description)
            return JsonResponse({'res': 'ثبت شد...'}, status=HTTP_200_OK)
        else:
...
def get_user_home_page_statistics(request):
    if Shop.objects.filter(FK_ShopManager=request.user).exists():
        ...
        for item in Message.objects.filter(Type=True):
            count += item.FK_Users.filter(FK_User=request.user,
                                          SeenStatus=False).count()
...
class UncompeletedFactors(ListAPIView):
    permission_classes = [IsAuthenticated]
    serializer_class = OnlyInvoiceSerializer
...
class ShopProductList(ListAPIView):
    ...
    def get_queryset(self):
        ...
        shop_products = shop_products.filter(Inventory__gt=inventory_from) if inventory_from else shop_products
        shop_products = shop_products.filter(Inventory__lt=inventory_to) if inventory_to else shop_products
