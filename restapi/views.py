...
class UserDashboardInfo(APIView):
    ...
    def get(self, request, shop_slug, format=None):
        ...
        completed_fators = Invoice.objects.completed_user_shop_invoices(user, shop_slug).count()
        uncompleted_fators = Invoice.objects.uncompleted_user_shop_invoices(user, shop_slug).count()
        uncomfirmed_factors = Invoice.objects.unconfirmed_user_shop_invoices(user, shop_slug).count()
        ...
        current_week_total_sell = InvoiceItem.objects.current_week_user_total_sell(user, shop_slug)

        # Total sell in last week
        last_week_total_sell = InvoiceItem.objects.last_week_user_total_sell(user, shop_slug)

        # Total sell in last month
        last_month_total_sell = InvoiceItem.objects.last_month_user_total_sell(user, shop_slug)
