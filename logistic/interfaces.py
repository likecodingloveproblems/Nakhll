    ...
    def generate_logistic_unit_list(self):
        ...
        if self.errors:
            ...
            raise ValidationError(_(
                'این محصولات هیچ روش ارسالی ندارند و'
                'از سبد خرید شما حذف خواهند شد<br>{}'
            ).format('<br>'.join([product.Title for product in self.errors])))
...
class ShopLogisticUnitInterface:
    ...
    def generate_shop_logistic_units(self):
        ...
        if self.all_products:
            ...
            for product in self.all_products:
                product_lus = self.get_product_available_logistic_units(
                ...
                if not product_lus:
                    ...
                    products_available_units.append(set(product_lus))
    ...
    def _get_pad_lu_name(self):
        """Get any pad logistic unit name for given shop

        If shop has any pad logistic unit, this method will return it's name.
        if not, it will return default pad logistic unit name.
        """
    ...
    def get_available_logistic_units(self, shop):
        ...
        available_logistic_units: list of all logistic units for given shop
            ...
            affect this calculation are:
                ...
                - Cart total_weight
        """
        if not models.ShopLogisticUnit.objects.filter(
                shop=shop, is_active=True).exists():
            return None
        filter_queryset = Q()
    ...
    def get_product_available_logistic_units(
        ...
        if exclude_pads:
            pay_times = models.ShopLogisticUnitCalculationMetric.PayTimes
            payer = models.ShopLogisticUnitCalculationMetric.PayerTypes
            pad_filter_queryset = Q(
                calculation_metric__pay_time=pay_times.AT_DELIVERY
            )
            pad_filter_queryset &= Q(
                calculation_metric__payer=payer.CUSTOMER
            )
    ...
    def calculate_logistic_price(self, unit):
        ...
        price += (
            unit.calculation_metric.price_per_extra_kilogram * extra_weight
        )
        return price
    ...
    def get_free_lus(self):
        ...
        payer = models.ShopLogisticUnitCalculationMetric.PayerTypes
        filter_queryset = Q(
            calculation_metric__pay_time=pay_times.WHEN_BUYING)
        filter_queryset &= Q(
            calculation_metric__payer=payer.SHOP)
        return self.available_logistic_units.filter(filter_queryset)
